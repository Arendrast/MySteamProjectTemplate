...
namespace Popcron
{
    public class Gizmos
    {
        ...
        /// </summary>
        public static async void DrawAsync<T>(Color? color, bool dashed, float time, params object[] args) where T : Drawer
        {
            if (time > 0)
            {
                while (time > 0)
                {
                    Draw<T>(color, dashed, args);
                    time -= Time.deltaTime;
        ...
        public static void Draw<T>(Color? color, bool dashed, params object[] args) where T : Drawer
        {
            ...
            if (drawer != null)
            {
                ...
                GizmosInstance.Submit(array, points, color, dashed);
            }
        }
        ...
        public static void Lines(Vector3[] lines, Color? color = null, bool dashed = false)
        {
            ...
            GizmosInstance.Submit(lines, lines.Length, color, dashed);
        }

        /// <summary>
        /// Draw line in world space.
        /// </summary>
        public static void Line(Vector3 a, Vector3 b, Color? color = null, bool dashed = false, float time = 0)
        {
            DrawAsync<LineDrawer>(color, dashed, time, a, b);
        }
        ...
        /// </summary>
        public static void Cube(Vector3 position, Quaternion rotation, Vector3 size, Color? color = null, bool dashed = false, float time = 0)
        {
            DrawAsync<CubeDrawer>(color, dashed, time, position, rotation, size);
        }
        ...
        /// </summary>
        public static void Sphere(Vector3 position, float radius, Color? color = null, bool dashed = false, int pointsCount = 16, float time = 0)
        {
            float offset = 0f;
            DrawAsync<PolygonDrawer>(color, dashed, time, position, pointsCount, radius, offset, Quaternion.Euler(90f, 0f, 0f));
            DrawAsync<PolygonDrawer>(color, dashed, time, position, pointsCount, radius, offset, Quaternion.Euler(0f, 0f, 0f));
            DrawAsync<PolygonDrawer>(color, dashed, time, position, pointsCount, radius, offset, Quaternion.Euler(0f, 90f, 90f));
        }
