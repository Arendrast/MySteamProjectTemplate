...
namespace ProjectName.NetworkModule.Runtime.Shared.Scripts.Scene
{
    namespace Scenes
    {
        public sealed class AddressableSceneProcessor : SceneProcessorBase
        {
            ...
            [SerializeField] private List<AssetReference> _sceneRefs = new();

            [SerializeField] private SerializedDictionary<string, AssetReference> _compiledAddressableReferences;
            ...
            /// </summary>
            private readonly List<UnityEngine.SceneManagement.Scene> _loadedScenes = new(4);
            ...
            private async void Awake()
            {
                ...
                for (int i = 0; i < _sceneRefs.Count; i++)
                {
                    _compiledAddressableReferences.Add((await _sceneRefs[i].GetAssetIdAsync()), _sceneRefs[i]);
                }
            }
            ...
            public AssetReference GetSceneRefByName(string sceneName)
            {
                ...
                _compiledAddressableReferences.TryGetValue(sceneName, out AssetReference assetReference);
                return assetReference;
            }

            public AssetReference GetSceneRefByGuid(string guid)
            {
                foreach (var item in _compiledAddressableReferences)
                {
            ...
            public override void BeginLoadAsync(string sceneName, LoadSceneParameters parameters)
            {
                ...
                // Try get reference
                if (!_compiledAddressableReferences.TryGetValue(sceneName, out var sceneReference))
                    throw new ArgumentException(
                        $"Scene with name: {sceneName} is not registered in AddressableSceneProcessor!",
                        nameof(sceneName));
                ...
                AsyncOperationHandle<SceneInstance> loadHandle =
                    Addressables.LoadSceneAsync(sceneReference, parameters, false);
            ...
            public override void BeginUnloadAsync(UnityEngine.SceneManagement.Scene scene)
            {
            ...
            public override List<UnityEngine.SceneManagement.Scene> GetLoadedScenes() => _loadedScenes;
            ...
            {
                UnityEngine.SceneManagement.Scene scene = _currentAsyncOperation.Result.Scene;
