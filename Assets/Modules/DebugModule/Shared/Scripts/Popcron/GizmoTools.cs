...
namespace Modules.SharedModule.Runtime.Shared.Scripts.Tools
{
    public static class GizmoTools
    {
        ...
        {
            DrawArc2D(center, 0f, Mathf.PI * 2f, radius);
        }

        public static void DrawArc2D(Vector2 center, float startAngle, float endAngle, float radius)
        {
            int segments = 12; 
            float step = (endAngle - startAngle) / segments;

            Vector3 prevPoint = center + new Vector2(Mathf.Cos(startAngle), Mathf.Sin(startAngle)) * radius;

            for (int i = 1; i <= segments; i++)
            {
                float currentAngle = startAngle + step * i;
                Vector3 nextPoint = center + new Vector2(Mathf.Cos(currentAngle), Mathf.Sin(currentAngle)) * radius;
