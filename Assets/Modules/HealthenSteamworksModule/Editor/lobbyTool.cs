...
namespace Modules.HealthenSteamworksModule.Editor
{
    public class LobbyIMGUITool
    {
        ...
        public void OnGUI()
        {
            ...
            {
                if (Matchmaking.Client.memberOfLobbies != null)
                {
                    if (Matchmaking.Client.memberOfLobbies.Count > 0)
                    {
                        DrawWindow();
                    }
                    else
        ...
        private void DrawWindow()
        {
            ...
            LobbyDetails(Matchmaking.Client.memberOfLobbies[lobbyIndex]);
            EditorGUILayout.EndScrollView();
        }

        private void ListLobbies()
        {
            for (int i = 0; i < Matchmaking.Client.memberOfLobbies.Count; i++)
            {
                var lobby = Matchmaking.Client.memberOfLobbies[i];
        ...
        private bool DrawLobbyEntry(LobbyData lobby, bool selected)
        {
            ...
            switch(typeString)
            {
                case "0": typeString = "Private"; break;
                case "1": typeString = "Friends Only"; break;
                case "2": typeString = "Public"; break;
                case "3": typeString = "Invisible"; break;
                case "4": typeString = "Private Unique (DO NOT USE)"; break;
