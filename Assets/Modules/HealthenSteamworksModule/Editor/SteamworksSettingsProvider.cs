...
namespace Modules.HealthenSteamworksModule.Editor
{
    public class SteamworksSettingsProvider : SettingsProvider
    {
        ...
        private void DrawConnectionSettings()
        {
            ...
            var address = Utilities.IPUintToString(settings.defaultServerSettings.ip);
            ...
            if (address != nAddress)
            {
                try
                {
                    var nip = Utilities.IPStringToUint(nAddress);
        ...
        private void DrawStatsList(SteamToolsSettings.AppSettings settings)
        {
            ...
            if (this[settings.editorName + "statsFoldout"])
            {
                ...
                EditorGUILayout.BeginHorizontal(EditorStyles.toolbar, GUILayout.ExpandWidth(true));
                GUI.contentColor = Colors.BrightGreen;
                ...
                for (int i = 0; i < settings.stats.Count; i++)
                {
                    ...
                    var terminate = false;
                    GUI.contentColor = Colors.ErrorRed;
            ...
            if (this[settings.editorName + "achievements"])
            {
                ...
                if (GUILayout.Button("Import", EditorStyles.toolbarButton, GUILayout.Width(50)))
                {
                    ...
                    try
                    {
                        ...
                        var names = StatsAndAchievements.Client.GetAchievementNames();
                ...
                for (int i = 0; i < settings.achievements.Count; i++)
                {
                    ...
                    if (UnityEngine.Application.isPlaying && App.Initialized)
            ...
            if (this["inventoryFoldout"])
            {
                ...
                if (GUILayout.Button("Import", EditorStyles.toolbarButton, GUILayout.Width(50)))
                {
                    ...
                    try
                    {
                        ...
                        Inventory.Client.OnSteamInventoryDefinitionUpdate.RemoveListener(settings.inventorySettings.HandleSettingsInventoryDefinitionUpdate);
                        Inventory.Client.OnSteamInventoryDefinitionUpdate.AddListener(settings.inventorySettings.HandleSettingsInventoryDefinitionUpdate);
                        Inventory.Client.LoadItemDefinitions();
                    }
                    catch
                    {
