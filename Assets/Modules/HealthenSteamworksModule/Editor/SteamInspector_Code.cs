...
namespace Modules.HealthenSteamworksModule.Editor
{
    public class SteamInspector_Code : EditorWindow
    {
        ...
        public void CreateGUI()
        {
            ...
            resetStatsButton.clicked += () => { StatsAndAchievements.Client.ResetAllStats(false); };
            refreshBoardsButton.clicked += RefreshAllLeaderboardEntries;
        }

        private void RefreshAllLeaderboardEntries()
        {
            if (refreshingScores || !Application.isPlaying || Leaderboards.Client.PendingDownloadScoreRequests > 0)
                return;

            var boards = Interface.GetBoards();
        ...
        private void OnGUI()
        {
            ...
            if (Application.isPlaying)
            {
                if (App.Initialized)
                {
                    var reportedId = App.Client.Id.m_AppId;

                    initializationField.text = "Initialized";
                    reportedAppId.text = App.Client.Id.m_AppId.ToString();
                    csteamId.text = User.Client.Id.ToString();
                    userName.text = User.Client.Id.Name;
                    userLevel.text = User.Client.Level.ToString();
                    var rp = User.Client.RichPresence;
                ...
                else if (App.HasInitializationError)
                {
        ...
        private void UpdateInventory()
        {
            ...
            while (inventoryContainer.childCount < settings.inventorySettings.items.Count)
            {
                ...
                grantButton.clicked += () =>
                {
                    ...
                    Inventory.Client.GenerateItems(new Steamworks.SteamItemDef_t[] { item.Id }, new uint[] { 1 }, (i) =>
                    {
                ...
                clearButton.clicked += () =>
                {
                    ...
                    foreach (var instance in item.Details)
                    {
                        if (instance.Quantity > 0)
                        {
                            ...
                            Inventory.Client.ConsumeItem(instance.ItemId, instance.Quantity, (i) =>
                            {
