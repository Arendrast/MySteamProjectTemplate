...
namespace Heathen.SteamworksIntegration.Modules.HealthenSteamworksModule.Runtime
{
    public class ItemShoppingCartManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            Inventory.Client.OnSteamMicroTransactionAuthorizationResponse.AddListener(HandleAuthorizationResponce);
        }

        private void OnDestroy()
        {
            Inventory.Client.OnSteamMicroTransactionAuthorizationResponse.RemoveListener(HandleAuthorizationResponce);
        }
        ...
        public string TotalPriceSymbolledString()
        {
            ...
            return Inventory.Client.LocalCurrencySymbol + (total * 0.01d).ToString("0.00");
        }
        ...
        {
            var total = TotalPrice();
            return (total * 0.01d).ToString("0.00") + " " + Inventory.Client.LocalCurrencyCode.ToString();
        }
        ...
        public void StartPurchase(Action<SteamInventoryStartPurchaseResult_t, bool> callback)
        {
            ...
            else
            {
                ...
                Inventory.Client.StartPurchase(itemDefs, itemQuan, (result, error) =>
                {
                    if(error)
                    {
