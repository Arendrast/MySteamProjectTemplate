    ...
    public class SteamInputActionGlyph : MonoBehaviour
    {
        ...
        public void RefreshImage()
        {
            if (m_Inspector != null && !string.IsNullOrEmpty(m_Inspector.Action.Name) && image != null)
            {
                ...
                {
                    if (Input.Client.connectedControllers.Count > 0)
                    {
                        var textures = m_Inspector.Action.GetInputGlyphs(Input.Client.connectedControllers[0], m_Inspector.Set);
                ...
                else if (!string.IsNullOrEmpty(m_Inspector.Layer.layerName))
                {
                    if (Input.Client.connectedControllers.Count > 0)
                    {
                        var textures = m_Inspector.Action.GetInputGlyphs(Input.Client.connectedControllers[0], m_Inspector.Layer);
                        if (textures.Length > 0)
                        {
                            image.texture = textures[0];
                        }
