...
namespace Heathen.SteamworksIntegration.Modules.HealthenSteamworksModule.Runtime
{
    ...
    public class LobbyManager : MonoBehaviour
    {
        ...
        {
            get => HasLobby && Matchmaking.Client.GetLobbyMemberData(Data, User.Client.Id, LobbyData.DataReady) == "true";
            set => Matchmaking.Client.SetLobbyMemberData(Data, LobbyData.DataReady, value.ToString().ToLower());
        }
        /// <summary>
        ...
        /// created by Heathen's tools such as <see cref="SteamLobbyData"/> and of course <see cref="Matchmaking.Client"/>
        /// </summary>
        ...
        {
            get => HasLobby ? Matchmaking.Client.GetLobbyMemberLimit(new CSteamID(Data)) : 0;
            set => Matchmaking.Client.SetLobbyMemberLimit(new CSteamID(Data), value);
        }
        /// <summary>
        ...
        /// </summary>
        public LobbyGameServer GameServer => Matchmaking.Client.GetLobbyGameServer(Data);
        ...
        {
            Matchmaking.Client.OnLobbyAskedToLeave.AddListener(HandleAskedToLeave);
            ...
            Matchmaking.Client.OnLobbyAuthenticationRequest.AddListener(HandleAuthRequest);
            Matchmaking.Client.OnLobbyChatMsg.AddListener(HandleChatMessage);
            Matchmaking.Client.OnLobbyInvite.AddListener(evtLobbyInvite.Invoke);
        }

        private void OnDisable()
        {
            Matchmaking.Client.OnLobbyAskedToLeave.RemoveListener(HandleAskedToLeave);
            ...
            Matchmaking.Client.OnLobbyGameCreated.RemoveListener(HandleGameServerSet);
            Matchmaking.Client.OnLobbyChatUpdate.RemoveListener(HandleChatUpdate);
            Matchmaking.Client.OnLobbyAuthenticationRequest.RemoveListener(HandleAuthRequest);
            Matchmaking.Client.OnLobbyChatMsg.RemoveListener(HandleChatMessage);
            Matchmaking.Client.OnLobbyInvite.RemoveListener(evtLobbyInvite.Invoke);
        }
        ...
        public bool SetType(ELobbyType type)
        {
            ...
            return Matchmaking.Client.SetLobbyType(Data, type);
        }
        /// <summary>
                    ...
                    if (result == EResult.k_EResultOK)
                    {
                        ...
                        foreach (var data in createArguments.metadata)
                        {
                            if (data.value.Contains("@[") && data.value.Contains("]"))
                            {
                                ...
                                while (Utilities.FindToken("@[", "]", workingString, out var resultString))
                                {
                                    switch (resultString)
                                    {
                                        case "@[userName]":
        ...
        public void Search(int maxResults)
        {
            ...
            foreach (var near in searchArguments.nearValues)
                Matchmaking.Client.AddRequestLobbyListNearValueFilter(near.key, near.value);

            foreach (var numeric in searchArguments.numericFilters)
                Matchmaking.Client.AddRequestLobbyListNumericalFilter(numeric.key, numeric.value, numeric.comparison);

            foreach (var text in searchArguments.stringFilters)
                Matchmaking.Client.AddRequestLobbyListStringFilter(text.key, text.value, text.comparison);

            Matchmaking.Client.AddRequestLobbyListResultCountFilter(maxResults);

            Matchmaking.Client.RequestLobbyList((r, e) =>
            {
                if (!e)
                {
                    evtFound?.Invoke(r);
        ...
        public bool SetLobbyData(string key, string value) => Matchmaking.Client.SetLobbyData(Data, key, value);
        public void SetMemberData(string key, string value) => Matchmaking.Client.SetLobbyMemberData(Data, key, value);
        public LobbyMemberData GetLobbyMember(UserData member) => new LobbyMemberData { lobby = Data, user = member };
        public string GetLobbyData(string key) => Matchmaking.Client.GetLobbyData(Data, key);
        public string GetMemberData(UserData member, string key) => Matchmaking.Client.GetLobbyMemberData(Data, member, key);
        public bool IsMemberReady(UserData member) => Matchmaking.Client.GetLobbyMemberData(Data, member, LobbyData.DataReady) == "true";
        public void KickMember(UserData member) => Data.KickMember(member);
        public void Invite(UserData user) => Matchmaking.Client.InviteUserToLobby(Data, user);
        public void Invite(uint FriendId) => Matchmaking.Client.InviteUserToLobby(Data, UserData.Get(FriendId));
        public LobbyMemberData[] Members => Matchmaking.Client.GetLobbyMembers(Data);
