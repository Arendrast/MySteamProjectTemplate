...
namespace Heathen.SteamworksIntegration.Modules.HealthenSteamworksModule.Runtime
{
    ...
    public class OverlayManager : MonoBehaviour
    {
        ...
        {
            get => Overlay.Client.NotificationPosition;
            set => Overlay.Client.NotificationPosition = value;
        }
        public Vector2Int NotificationInset
        {
            get => Overlay.Client.NotificationInset;
            set => Overlay.Client.NotificationInset = value;
        }
        ...
        private void OnEnable()
        {
            ...
            {
                App.onSteamInitialized.AddListener(EnabledProcess);
            }
        }
        ...
        private void EnabledProcess()
        {
            ...
            Overlay.Client.OnGameOverlayActivated.AddListener(evtOverlayActivated.Invoke);
            Overlay.Client.OnGameServerChangeRequested.AddListener(evtGameServerChangeRequested.Invoke);
            Overlay.Client.OnGameLobbyJoinRequested.AddListener(evtGameLobbyJoinRequested.Invoke);
            Overlay.Client.OnGameRichPresenceJoinRequested.AddListener(evtRichPresenceJoinRequested.Invoke);
        }

        private void OnDisable()
        {
            Overlay.Client.OnGameOverlayActivated.RemoveListener(evtOverlayActivated.Invoke);
            Overlay.Client.OnGameServerChangeRequested.RemoveListener(evtGameServerChangeRequested.Invoke);
            Overlay.Client.OnGameLobbyJoinRequested.RemoveListener(evtGameLobbyJoinRequested.Invoke);
            Overlay.Client.OnGameRichPresenceJoinRequested.RemoveListener(evtRichPresenceJoinRequested.Invoke);
        }
        ...
        private void Update()
        {
            if (App.Initialized)
            {
                if (notificationPosition != Overlay.Client.NotificationPosition)
                {
                    notificationPosition = NotificationPosition;
                    Debug.LogWarning("Notification Position cannot be updated from the inspector at runtime.");
                }
                if (notificationInset != Overlay.Client.NotificationInset)
                {
                    notificationInset = NotificationInset;
        ...
        public void OpenDialogName(string dialog) => Overlay.Client.Activate(dialog);
        /// <summary>
        /// Activates the Steam Overlay to a specific dialog.
        /// </summary>
        /// <param name="dialog">The dialog to open. Valid options are: "friends", "community", "players", "settings", "officialgamegroup", "stats", "achievements".</param>
        public void OpenDialog(OverlayDialog dialog) => Overlay.Client.Activate(dialog);
        /// <summary>
        ...
        public void OpenLobbyInvite(LobbyData lobbyId) => Overlay.Client.ActivateInviteDialog(lobbyId);
        public void OpenLobbyInvite(SteamLobbyData lobby)
        {
            
            if (lobby != null && lobby.Data.IsValid)
                Overlay.Client.ActivateInviteDialog(lobby.Data);
        }
        public void OpenConnectStringInvite(string connectionString) => Overlay.Client.ActivateInviteDialog(connectionString);
        public void OpenRemotePlayInvite(LobbyData lobbyId) => Overlay.Client.ActivateRemotePlayInviteDialog(lobbyId);
        public void OpenRemotePlayInvite(SteamLobbyData lobby)
        {
            if (lobby != null && lobby.Data.IsValid)
                Overlay.Client.ActivateRemotePlayInviteDialog(lobby.Data);
        }
        ...
        public void OpenStore(AppData appID, EOverlayToStoreFlag flag) => Overlay.Client.Activate(appID, flag);
        public void OpenUserProfile(SteamUserData user)
        {
            if (user != null && user.Data.IsValid)
                Overlay.Client.Activate("steamid", user.Data);
        }
        public void OpenUserChat(SteamUserData user)
        {
            if (user != null && user.Data.IsValid)
                Overlay.Client.Activate("chat", user.Data);
        }
        public void OpenUserJoinTrade(SteamUserData user)
        {
            if (user != null && user.Data.IsValid)
                Overlay.Client.Activate("jointrade", user.Data);
        }
        public void OpenUserStats(SteamUserData user)
        {
            if (user != null && user.Data.IsValid)
                Overlay.Client.Activate("stats", user.Data);
        }
        public void OpenUserAchievements(SteamUserData user)
        {
            if (user != null && user.Data.IsValid)
                Overlay.Client.Activate("achievements", user.Data);
        }
        public void OpenUserAddFriend(SteamUserData user)
        {
            if (user != null && user.Data.IsValid)
                Overlay.Client.Activate("friendadd", user.Data);
        }
        public void OpenUserRemoveFriend(SteamUserData user)
        {
            if (user != null && user.Data.IsValid)
                Overlay.Client.Activate("friendremove", user.Data);
        }
        public void OpenUserAcceptFriendRequest(SteamUserData user)
        {
            if (user != null && user.Data.IsValid)
                Overlay.Client.Activate("friendrequestaccept", user.Data);
        }
        public void OpenUserIgnoreFriendRequest(SteamUserData user)
        {
            if (user != null && user.Data.IsValid)
                Overlay.Client.Activate("friendrequestignore", user.Data);
        }
        /// <summary>
        ...
        public void OpenUser(string dialog, CSteamID steamId) => Overlay.Client.Activate(dialog, steamId);
        /// <summary>
        /// Activates Steam Overlay to a specific dialog.
        /// </summary>
        /// <param name="dialog">The dialog to open.</param>
        /// <param name="steamId">The Steam ID of the context to open this dialog to.</param>
        public void OpenUser(FriendDialog dialog, CSteamID steamId) => Overlay.Client.Activate(dialog.ToString(), steamId);
        /// <summary>
        ...
        public void OpenWebPage(string url) => Overlay.Client.ActivateWebPage(url);
    }
