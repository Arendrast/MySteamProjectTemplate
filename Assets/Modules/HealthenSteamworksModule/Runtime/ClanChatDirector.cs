...
namespace Heathen.SteamworksIntegration.Modules.HealthenSteamworksModule.Runtime
{
    ...
    public class ClanChatDirector : MonoBehaviour
    {
        ...
        {
            Clans.Client.OnChatMessageReceived.AddListener(HandleNewMessage);
            Clans.Client.OnGameConnectedChatJoin.AddListener(HandleJoined);
            Clans.Client.OnGameConnectedChatLeave.AddListener(HandleLeave);
        }

        private void OnDisable()
        {
            Clans.Client.OnChatMessageReceived.RemoveListener(HandleNewMessage);
            Clans.Client.OnGameConnectedChatJoin.RemoveListener(HandleJoined);
            Clans.Client.OnGameConnectedChatLeave.RemoveListener(HandleLeave);
        }

        public void Join(ClanData clan)
        {
            Clans.Client.JoinChatRoom(clan, (result, error) =>
            {
                if (!error)
                    chatRoom = result;
                else
