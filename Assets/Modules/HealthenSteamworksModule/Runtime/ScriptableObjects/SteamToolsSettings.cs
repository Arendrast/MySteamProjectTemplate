...
namespace Heathen.SteamworksIntegration.Modules.HealthenSteamworksModule.Runtime.ScriptableObjects
{
    public class SteamToolsSettings : ScriptableObject
    {
        ...
        private string GenerateWrapperCode()
        {
            ...
            sb.AppendLine($"             {typeof(App).FullName}.{nameof(App.onSteamInitialized)}.{nameof(App.onSteamInitialized.AddListener)}(HandleInitialized);");
            sb.AppendLine("#if UNITY_SERVER");
            sb.AppendLine($"             {typeof(App.Server).FullName.Replace('+', '.')}.{nameof(App.Server.Initialize)}(AppId, ServerConfiguration);");
            sb.AppendLine("#else");
            ...
            sb.AppendLine($"#endif");
            sb.AppendLine();
            sb.AppendLine($"             {typeof(App.Client).FullName.Replace('+', '.')}.{nameof(App.Client.Initialize)}(AppId, actions.ToArray());");
            sb.AppendLine($"#endif");
            sb.AppendLine("        }");
