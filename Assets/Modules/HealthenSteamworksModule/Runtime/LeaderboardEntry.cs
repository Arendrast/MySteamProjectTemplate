...
namespace Heathen.SteamworksIntegration.Modules.HealthenSteamworksModule.Runtime
{
    public class LeaderboardEntry
    {
        ...
        public void GetAttachedUgc<T>(Action<T, bool> callback = null)
        {
            ...
            {
                RemoteStorage.Client.UGCDownload(UgcHandle, 0, (dr, de) =>
                {
                    if (!de && dr.m_eResult == EResult.k_EResultOK)
                    {
                        ...
                        if (callback != null)
                        {
                            var buffer = RemoteStorage.Client.UGCRead(dr.m_hFile);
        ...
        public bool StartUgcDownload(uint priority, Action<RemoteStorageDownloadUGCResult_t, bool> callback)
        {
            if (UgcHandle != UGCHandle_t.Invalid)
            {
                RemoteStorage.Client.UGCDownload(UgcHandle, priority, (p, e) =>
                {
