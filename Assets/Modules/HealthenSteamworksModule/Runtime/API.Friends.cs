...
namespace Heathen.SteamworksIntegration.Modules.HealthenSteamworksModule.Runtime
{
    ...
    public static class Friends
    {
        public static class Client
        {
            ...
            static void Init()
            {
                m_OnFriendMessageReceived = new();
                m_IsListeningForFriendMessages = false;
                m_OnPersonaStateChange = new();
                m_OnFriendRichPresenceUpdate = new();
                m_PendingLinks = new();

                if (m_LoadedImages.Count > 0)
                    UnloadAvatarImages();

                m_LoadedImages = new();
                m_UserAvatarMapping = new();
            ...
            /// </remarks>
            public static GameConnectedFriendChatMsgEvent OnGameConnectedFriendChatMsg
            {
                get
                {
                    m_GameConnectedFriendChatMsg_t ??= Callback<GameConnectedFriendChatMsg_t>.Create((result) =>
                        {
                            ...
                            if (count > 0)
                                m_OnFriendMessageReceived.Invoke(result.m_steamIDUser, message, type);
                        });

                    return m_OnFriendMessageReceived;
                }
            }
            ...
            /// </summary>
            public static FriendRichPresenceUpdateEvent OnFriendRichPresenceUpdate
            {
                get
                {
                    m_FriendRichPresenceUpdate_t ??= Callback<FriendRichPresenceUpdate_t>.Create((r) => m_OnFriendRichPresenceUpdate.Invoke(r));

                    return m_OnFriendRichPresenceUpdate;
                }
            }

            public static PersonaStateChangeEvent OnPersonaStateChange
            {
                get
                {
                    m_PersonaStateChange_t ??= Callback<PersonaStateChange_t>.Create(HandlePersonaStateChange);

                    return m_OnPersonaStateChange;
                }
            }
            ...
            {
                get => m_IsListeningForFriendMessages;
                set
                {
                    SteamFriends.SetListenForFriendsMessages(value);
                    m_IsListeningForFriendMessages = value;
                }
            }
            /// <summary>
            ...
            private static GameConnectedFriendChatMsgEvent m_OnFriendMessageReceived = new GameConnectedFriendChatMsgEvent();
            private static bool m_IsListeningForFriendMessages = false;
            private static PersonaStateChangeEvent m_OnPersonaStateChange = new PersonaStateChangeEvent();
            private static FriendRichPresenceUpdateEvent m_OnFriendRichPresenceUpdate = new FriendRichPresenceUpdateEvent();

            private static List<ImageRequestCallbackLink> m_PendingLinks = new List<ImageRequestCallbackLink>();
            private static Dictionary<int, Texture2D> m_LoadedImages = new Dictionary<int, Texture2D>();
            private static Dictionary<UserData, Texture2D> m_UserAvatarMapping = new Dictionary<UserData, Texture2D>();
            ...
            public static void UnloadAvatarImage(Texture2D image)
            {
                ...
                foreach (var kvp in m_UserAvatarMapping)
                {
                    if (kvp.Value == image)
                    {
                        user = kvp.Key;
                ...
                m_UserAvatarMapping.Remove(user);
            }
            /// <summary>
            ...
            {
                if (m_UserAvatarMapping.ContainsKey(id))
                    return m_UserAvatarMapping[id];
                else
                    return null;
            }
            ...
            private static void HandleAvatarImageLoaded(AvatarImageLoaded_t results)
            {
                ...
                {
                    var image = m_LoadedImages[results.m_iImage];

                    foreach (var links in m_PendingLinks)
                    {
                    ...
                    m_PendingLinks.RemoveAll(p => p.owner == results.m_steamID);
                }
                else
                {
                ...
                switch (results.m_nChangeFlags)
                {
                    case EPersonaChange.k_EPersonaChangeAvatar:
                        ...
                        if (avatarData > 0)
                        {
                            ...
                            {
                                var image = m_LoadedImages[avatarData];
                                foreach (var links in m_PendingLinks)
                                {
                                ...
                                m_PendingLinks.RemoveAll(p => p.owner == steamId);
                            }
                        }
                        break;
                }

                m_OnPersonaStateChange.Invoke(results);
            }
            ...
            private static bool LoadAvatar(int imageHandle, CSteamID user)
            {
                if (SteamUtils.GetImageSize(imageHandle, out uint width, out uint height))
                {
                    ...
                    if (m_UserAvatarMapping.ContainsKey(user))
                        m_UserAvatarMapping[user] = pointer;
                    else
                        m_UserAvatarMapping.Add(user, pointer);
