...
namespace Heathen.SteamworksIntegration.Modules.HealthenSteamworksModule.Runtime
{
    ...
    public class SteamLeaderboardData : MonoBehaviour
    {
        ...
        {
            Interface.OnReady -= Interface_OnReady;

            if (!m_Data.IsValid)
            {
                if(!string.IsNullOrEmpty(apiName))
                {
                    m_Data = Interface.GetBoard(apiName);
                    if (!m_Data.IsValid)
                    {
                        if (createIfMissing)
                            Leaderboards.Client.FindOrCreate(Data.apiName, (ELeaderboardSortMethod)createWithSort, (ELeaderboardDisplayType)createAsDisplay, (data, ioError) =>
                            {
                                if (!ioError)
                                {
                                    m_Data = data;
