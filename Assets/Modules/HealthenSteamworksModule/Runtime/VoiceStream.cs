...
namespace Heathen.SteamworksIntegration.Modules.HealthenSteamworksModule.Runtime
{
    ...
    public class VoiceStream : MonoBehaviour
    {
        ...
        /// <summary>
        /// Players a received Steamworks Voice package through the <see cref="outputSource"/> <see cref="AudioSource"/>.
        /// </summary>
        ...
        public void PlayVoiceData(byte[] buffer)
        {
            ...
            var result = Voice.Client.DecompressVoice(buffer, destBuffer, out bytesWritten, (uint)sampleRate);
            var timeStamp = DateTime.Now;

            if (result == EVoiceResult.k_EVoiceResultBufferTooSmall)
            {
                destBuffer = new byte[bytesWritten];
                result = Voice.Client.DecompressVoice(buffer, destBuffer, out bytesWritten, (uint)sampleRate);
            }
...
#if UNITY_EDITOR
    [UnityEditor.CustomEditor(typeof(VoiceStream), true)]
    public class VoiceStreamEditor : UnityEditor.Editor
    {
