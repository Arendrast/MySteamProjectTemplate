        ...
        public void RefreshName()
        {
            if (m_Inspector != null && !string.IsNullOrEmpty(m_Inspector.Action.Name) && label != null)
            {
                if (m_Inspector.Set.Handle > 0)
                {
                    ...
                    {
                        var names = m_Inspector.Action.GetInputNames(Input.Client.connectedControllers[0], m_Inspector.Set);
                        if (names.Length > 0)
                        {
                            label.text = names[0];
                        }
                ...
                else if (!string.IsNullOrEmpty(m_Inspector.Layer.layerName))
                {
                    if (Input.Client.connectedControllers.Count > 0)
                    {
                        var names = m_Inspector.Action.GetInputNames(Input.Client.connectedControllers[0], m_Inspector.Layer);
                        if (names.Length > 0)
                        {
                            label.text = names[0];
                        }
